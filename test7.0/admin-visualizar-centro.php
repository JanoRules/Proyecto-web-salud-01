...
$sql = "SELECT id, nombre, atencion, tipo, capacidad_total, 
               personas_atencion_baja, personas_atencion_media, personas_atencion_inmediata,
...
// Detectar si algún centro alcanza el 90% o 100% de capacidad
while ($row = $result->fetch_assoc()) {
    if ($row['porcentaje_ocupacion'] >= 90) {
        // Mensaje dinámico dependiendo del porcentaje
        if ($row['porcentaje_ocupacion'] == 100) {
            // Mensaje para cuando el centro de salud esté al 100% de capacidad
            $mensaje = "En el centro de salud " . $row['nombre'] . " están a su 100% de capacidad, por favor, redirigirse a otro centro de salud.";
        } else {
            // Mensaje para cuando el centro de salud esté al 90% de capacidad
            $mensaje = "En el centro de salud " . $row['nombre'] . " están a su 90% de capacidad.";
        }

        // Verificar si ya existe esta notificación en la base de datos para evitar duplicados
        $notifSql = "INSERT INTO notificaciones (mensaje, visto) 
                     SELECT ?, FALSE 
                     WHERE NOT EXISTS (
                         SELECT 1 FROM notificaciones 
                         WHERE mensaje = ? AND visto = FALSE
                     )";
        $notifStmt = $conn->prepare($notifSql);
        $notifStmt->bind_param("ss", $mensaje, $mensaje);
        $notifStmt->execute();
    }
}

// Eliminar la lógica de inserción de notificaciones por actualizaciones de centros
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['updateCenter'])) {
    ...
    // Solo actualizar los datos sin insertar notificaciones
